...
async function crearMapa(tipo, mapContainer){
    ...
    switch (tipo) {
            ...
            // Agregar sensores
            getSensorsInfo(map, markersSensors, periodoObservacion);
...
async function actualizarFecha(map, markers, markersSensors, regionCode, date, type) {
    ...
    getSensorsInfo(map, markersSensors, date);
    switch (type) {
        case "general":
...
function addLayerSensor(map, markers, observation){
    ...
    popupContent += "<strong>Sensor:</strong> " + observation.name + "<br>" +
        ...
        "<strong>Coordinates:</strong> <br> Latitude: " + observation.lat + ", Longitude: " + observation.long + ", Altitude: " + observation.altitude + "<br>" +
...
async function getSensorsInfo(map, markers, date) {
    const apiUrlTemperature = "http://localhost:8081/temperature";
    const apiUrlTurbidity = "http://localhost:8082/turbidity";
    const apiUrlChlorophyll = "http://localhost:8083/chlorophyll";

    let startDate = getDate(date);
    var temperature = await getData(apiUrlTemperature, startDate);
    var turbidity = await getData(apiUrlTurbidity, startDate);
    var chlorophyll = await getData (apiUrlChlorophyll, startDate);
    getSensors(map, markers, temperature);
    getSensors(map, markers, turbidity);
    getSensors(map, markers, chlorophyll);
}
// Funcion dibujar sensores
function getSensors(map, markers, sensor) {
    let total = 0, media = 0;
    let name = "", lastDate = "", lat = "", long = "", alt = "", units = "";
    let allRegisters = [];
    sensor.forEach(register => {
        if (register) {
            total++;
...
function addRegisters(values) {
    ...
    for (let i in values.all) {
        if(i == 0){
           document.getElementById("registrosColumnas").innerHTML = "<div class='rows main'><div>Sensor</div><div class='idubication'>Id</div><div>Latitude</div><div>Longitude</div><div>Altitude</div><div>Value</div><div>Units</div><div>Date</div></div>";
        }
        if(i%2 == 0){
        ...
        registros += "<div class='rows " + classname + "'><div>" + value.name + "</div><div class='idubication'>" + value.id + "</div><div>" + value.lat + "</div><div>" + value.long + "</div><div>" + value.altitude + "</div><div>" + value.value + "</div><div>" + value.units + "</div><div>" + value.date + "</div></div>";
    }
